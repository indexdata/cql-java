...
public class CQLParser {
     ...
     * @param allowKeywordTerms when false registered keywords are disallowed in
     *                          unquoted terms
     */
     ...
     * is registered the parser is no longer strictly compliant with the chosen
     * spec.
    ...
    /**
     * Unregisters previously registered custom relation in this instance of the
     * parser.
     *
     * @param relation
    ...
    private CQLNode parseTerm(String index, CQLRelation relation)
        ...
        while (true) {
            ...
            // render relation
            String relstr = (lexer.what() == CQLTokenizer.TT_WORD ? lexer.value() : lexer.render(lexer.what(), false));
            // we have relation, but it only makes sense if preceded by a single term
            if (all.length() > first.length()) {
                throw new CQLParseException("unexpected relation '" + relstr + "'", lexer.pos());
            }
            index = first;
    ...
    private CQLNode parsePrefix(String index, CQLRelation relation,
        ...
        CQLNode node = topLevel ? parseTopLevelPrefixes(index, relation) : parseQuery(index, relation);
